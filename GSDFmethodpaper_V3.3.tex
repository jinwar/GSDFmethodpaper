...
Frequency Time Analysis (FTAN) method is widely used as one of the classical seismic analysis procedures in many studies\cite{Levshin:1992ve,Levshin:2001ad,Lin:2011fw,Yang:2011sw}. For each frequency, the method measures the phase and amplitude at the envelop function maximum.

The method relies on the detection of a continuous group delay travel time curve by tracking the maximum amplitude of the envelope function at each frequencies. Then the phase and amplitude on this curve is measured. Finally, phase delay from the source to the station is then calculated by assuming a reference model. 
